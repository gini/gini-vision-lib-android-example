    ...
    stages {
        ...
        stage('Upload to Hockeyapp') {
            ...
            steps {
                ...
                step([$class: 'HockeyappRecorder', applications: [[apiToken: HOCKEYAPP_API_TOKEN, downloadAllowed: true, dsymPath: 'app/build/outputs/mapping/newest/release/mapping.txt', filePath: 'app/build/outputs/apk/newest/release/app-newest-release.apk', mandatory: false, notifyTeam: false, releaseNotesMethod: [$class: 'FileReleaseNotes', fileName: 'changelog.md', isMarkdown: true], uploadMethod: [$class: 'AppCreation', publicPage: true]]], debugMode: false, failGracefully: false])
            }
            post {
                always {
